...
static void log(const char* szFile, const char* szLog, size_t log_len)
{
	...
	time(&tt);
	ret = base::crt::localtime(&st, &tt);
	if (ret != 0)
	{
		return;
	}

	ret = base::crt::sprintf(
		szTime,
		sizeof(szTime),
...
static void logf(const char* szFileName, const char* fmt, ...)
{
	...
	ret = base::crt::vsprintf(log_buffer, sizeof(log_buffer), fmt, arglist);
	va_end(arglist);
	if (ret < 0)
	{
		return;
...
#error Architecture is not supported.
		...
		if (name_ok && addr_ok)
		{
			int ret = base::crt::sprintf(output + pos, size - pos,
				"Frame %d: %s %s line:%u\r\n",
				frame, symbol->Name, line.FileName, line.LineNumber
			);
			if (ret < 0)
